...
makeTypesL2NewSim :: Maybe TermType -> String
makeTypesL2NewSim t = case t of
                Just Bool -> "(forall C:*. C -> C -> C)"
                Just Nat  -> "(forall C:*. (C -> C) -> C -> C)"

                Just (Pair t1 t2) -> "(forall C:*. (" 
                                        <> makeTypesL2NewSim (Just t1) <> " -> "
                                        <> makeTypesL2NewSim (Just t2) <> " -> C) -> C) " 


                Just (Func t1 t2) -> "(" <> makeTypesL2NewSim (Just t1) <> "->" <> makeTypesL2NewSim (Just t2) <> ")"
                Nothing -> "ERRO DE TIPO"
...
makeTypesL2DefsNewSim :: Maybe TermType -> String
makeTypesL2DefsNewSim t = case t of
                Just Bool -> "Bool"
                Just Nat  -> "Nat"

                Just (Pair t1 t2) -> "(forall C:*. (" 
                                        <> makeTypesL2DefsNewSim (Just t1) <> " -> "
                                        <> makeTypesL2DefsNewSim (Just t2) <> " -> C) -> C) " 


                Just (Func t1 t2) -> "(" <> makeTypesL2DefsNewSim (Just t1) <> "->" <> makeTypesL2DefsNewSim (Just t2) <> ")"
                Nothing -> "ERRO DE TIPO"
...
termToL2NewSim ∷ Term -> Env -> String
termToL2NewSim expr env = case expr of
            T_true  -> "(\\C:*.\\a:C.\\b:C.a)"
            T_false -> "(\\C:*.\\a:C.\\b:C.b)"
            T_var id -> "x_" <> id
            T_num n -> "(\\C:*.\\f:C->C.\\x:C." <> (makeNatural n) <> ")"

            T_func id t_var e1 -> "(\\x_" <> id <> ": " <> makeTypesL2NewSim (Just t_var) <> ". " 
                                  <> termToL2NewSim e1 (update env id t_var) <> ")"

            T_func_system id t_var e1 -> "(\\" <> id <> ": " <> makeTypesL2NewSim (Just t_var) <> ". " 
                                  <> termToL2NewSim e1 (update env id t_var) <> ")"
            T_var_system id -> id

            T_app e1 e2 -> "(" <> termToL2NewSim e1 env <> " " <> termToL2NewSim e2 env <> ")"

            T_let id t_var e1 e2 -> termToL2NewSim (T_app (T_func id t_var e2) e1) env


            (T_if e1 e2 e3) ->   "((\\D:*. \\c:(forall C:*. C -> C -> C). \\a:D. \\b:D. (c D) a b)" <> " " 
                                <> (makeTypesL2NewSim $ typeInfer env e2) <> " " 
                                <> termToL2NewSim e1 env <> " "
                                <> termToL2NewSim e2 env <> " "
                                <> termToL2NewSim e3 env <> ")"

            (T_pair e1 e2) -> "((\\A:*. \\B:*. \\a: A. \\b: B. \\C:*. \\f: A->B->C. f a b)" <> " "
                              <> (makeTypesL2NewSim $ typeInfer env e1) <> " "
                              <> (makeTypesL2NewSim $ typeInfer env e2) <> " "
                              <> termToL2NewSim e1 env <> " "
                              <> termToL2NewSim e2 env <> ")"
            ...
            T_fst e1 ->  "((\\A:*. \\B:*. \\p: (forall C:*. (A -> B -> C) -> C). p A (\\a: A.\\b: B. a))"
                      <> (case typeInfer env e1 of 
                          Just (Pair t1 t2) -> " " <> (makeTypesL2NewSim $ Just t1) <> " " <> (makeTypesL2NewSim $ Just t2) <> " "
                          _ -> "Erro de Tipo [PARES]"
                          )
                      <> termToL2NewSim e1 env <> ")"
            ...
            T_binop Sub e1 e2 -> "(" <> termToL2NewSim subTerm env <> " "
                                    <> termToL2NewSim e1 env <> " "
                                    <> termToL2NewSim e2 env <> ")"
            
            T_binop Eq e1 e2 -> "(" <> termToL2NewSim eqTerm env <> " "
                                    <> termToL2NewSim e1 env <> " "
                                    <> termToL2NewSim e2 env <> ")"

            T_binop Ne e1 e2 -> "(" <> termToL2NewSim neTerm env <> " "
                                    <> termToL2NewSim e1 env <> " "
                                    <> termToL2NewSim e2 env <> ")"

            T_binop Gt e1 e2 -> "(" <> termToL2NewSim gtTerm env <> " "
                                    <> termToL2NewSim e1 env <> " "
                                    <> termToL2NewSim e2 env <> ")"

            T_binop Lt e1 e2 -> "(" <> termToL2NewSim ltTerm env <> " "
                                    <> termToL2NewSim e1 env <> " "
                                    <> termToL2NewSim e2 env <> ")"

            T_natRec e1 e2 e3 -> "(" <> termToL2NewSim e1 env
                                <> " " <> (makeTypesL2NewSim $ typeInfer env e3) <> " "
                                <> termToL2NewSim e2 env <> " "
                                <> termToL2NewSim e3 env <> ")"
...
termToNewSimL2Defs ∷ Term -> Env -> String
termToNewSimL2Defs expr env = case expr of
            T_true  -> "true"
            T_false -> "false"
            T_var id -> "x_" <> id
            T_num n -> toStringAs decimal n 

            T_func id t_var e1 -> "(\\x_" <> id <> ": " <> makeTypesL2DefsNewSim (Just t_var) <> ". " 
                                  <> termToNewSimL2Defs e1 (update env id t_var) <> ")"

            T_app e1 e2 -> "(" <> termToNewSimL2Defs e1 env <> " " <> termToNewSimL2Defs e2 env <> ")"

            T_let id t_var e1 e2 -> termToNewSimL2Defs (T_app (T_func id t_var e2) e1) env


            (T_if e1 e2 e3) ->   "(if " 
                                <> (makeTypesL2DefsNewSim $ typeInfer env e2) <> " " 
                                <> termToNewSimL2Defs e1 env <> " "
                                <> termToNewSimL2Defs e2 env <> " "
                                <> termToNewSimL2Defs e3 env <> ")"

            (T_pair e1 e2) -> "(pair "
                              <> (makeTypesL2DefsNewSim $ typeInfer env e1) <> " "
                              <> (makeTypesL2DefsNewSim $ typeInfer env e2) <> " "
                              <> termToNewSimL2Defs e1 env <> " "
                              <> termToNewSimL2Defs e2 env <> ")"
            ...
            T_fst e1 ->  "(fst "
                      <> (case typeInfer env e1 of 
                          Just (Pair t1 t2) -> " " <> (makeTypesL2DefsNewSim $ Just t1) <> " " <> (makeTypesL2DefsNewSim $ Just t2) <> " "
                          _ -> "Erro de Tipo [PARES]"
                          )
                      <> termToNewSimL2Defs e1 env <> ")"
            ...
            T_natRec e1 e2 e3 -> "(natRec "
                                <> (makeTypesL2DefsNewSim $ typeInfer env e3) <> " "
                                <> termToNewSimL2Defs e1 env <> " "
                                <> termToNewSimL2Defs e2 env <> " "
                                <> termToNewSimL2Defs e3 env <> ")"
...
makeDefNewSimL2 :: String -> String
makeDefNewSimL2 str = case str of 
    "true"  -> "  true    = \\C:*. \\a: C. \\b: C. a;"
    "false" -> "  false   = \\C:*. \\a: C. \\b: C. b;"
    "if"    -> "  if      = \\D:*. \\c: Bool. \\a: D. \\b: D. c D a b;"
    "pair"  -> "  pair    = \\A:*. \\B:*. \\a: A. \\b: B. \\C:*. \\f: A->B->C. f a b;"
...
makeDefsBlockNewSim l =  "typedef\n"
    ...
    <> "end\n"
    <> (case l of
            (_:_) -> "let\n" <> (makeDefsUsed makeDefNewSimL2 l)  <> "in\n\n"
            Nil   -> "\n\n")
...
makeL2NewSim :: Term → String
makeL2NewSim expr = case typeInfer emptyEnv expr of 
                    Just _ -> termToL2NewSim expr emptyEnv
                    Nothing -> if (expr == T_error) then "Sintaxe Incorreta" else "Erro de Tipo"

makeL2DefsNewSim :: Term → String
makeL2DefsNewSim expr = case typeInfer emptyEnv expr of 
                    Just _ -> makeDefsBlockNewSim (listTermsUsed expr Nil) <> termToNewSimL2Defs expr emptyEnv
                    Nothing -> if (expr == T_error) then "Sintaxe Incorreta" else "Erro de Tipo"